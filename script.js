...
async function loadHistoricalStats() {
  try {
    ...
    document.querySelector('#statOnline').textContent = stats.onlinePlayers ?? '—';
    document.querySelector('#statWeek').textContent = stats.uniquePlayers7d ?? '—';
    document.querySelector('#statMonth').textContent = stats.uniquePlayers30d ?? '—';
    document.querySelector('#statRecord').textContent = stats.bestPeakPlayers ?? '—';
    if (stats.updatedAt) {
      const date = new Date(stats.updatedAt);
      document.querySelector('#statsUpdated').textContent = `Ostatnia aktualizacja: ${date.toLocaleString('pl-PL')}`;
    }
  } catch {
    document.querySelector('#statsUpdated').textContent = 'Statystyki chwilowo niedostępne';
  }
}
...
setInterval(loadHistoricalStats, 300000);
